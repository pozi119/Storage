...
extension Orm: Redisable where T: Equatable {
    ...
    public func primaryKeyValue(of key: String) -> [String: Binding]? {
        guard config.primaries.count > 0 else { return nil }
        let values = key.components(separatedBy: separator)
        guard values.count == config.primaries.count else { return nil }
        var result: [String: Binding] = [:]
        for i in 0 ..< values.count {
            let k = config.primaries[i]
            let v = values[i]
            result[k] = v
        }
        return result
    ...
    public func keyValue(of value: Value) -> (String, [String: Binding]) {
        ...
        guard config.primaries.count > 0,
            let _val = try? encoder.encode(value) as? [String: Binding] else { return result }
        let array = config.primaries.map { String(describing: _val[$0] ?? "") }
    ...
    private func constraint(lower: String?, upper: String?, bounds: Bounds) -> Where? {
        guard config.primaries.count > 0 else { return nil }
        let count = config.primaries.count
        ...
        var condition = Where("")
        for i in 0 ..< count {
            let pk = config.primaries[i], l = lvalues[i], u = uvalues[i]
    ...
    public func keys(lower: Key? = nil,
        ...
        guard config.primaries.count > 0,
            let condition = constraint(lower: lower, upper: upper, bounds: bounds) else { return [] }
        let orderBy = OrderBy(desc ? config.primaries.map { $0 + " DESC" } : config.primaries)
        let items = xFind(condition, fields: Fields(config.primaries), orderBy: orderBy, limit: Int64(limit ?? 0))
